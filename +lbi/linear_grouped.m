function obj = linear_grouped(data, opt, group_index, verbose)
% DESCRIPTION
% Return regularization path with group sparsity under linear model.
...
%% Initialize z(0),\beta(0) and \beta_tilde(0) to zeros %%
if opt.intercept
    X_tilde = [ones(n, 1), X];
    beta_tilde = lbi.linear_minimize(X_tilde,y);
    beta0 = beta_tilde(1);
    beta = beta_tilde(2:end);
else
    beta0 = 0;
    beta = lbi.linear_minimize(X,y,D,nu);
end
z = zeros(p, 1);
...
for step_cur = 1:steps_remain
    ...
    %% update \beta(0),\beta,z %%
    if opt.intercept
        d_beta0 = beta0 + mean(X * beta - y);
    end
    if opt.intercept
